...
\begin{figure}[h]
  \begin{align*}
    ...
    \pipe \kw{def} X(\overline{X : T}) : T = E
...
\begin{figure}[h]
  \begin{mathpar}
    ...
    \inferrule{\\}{\kw{type} X \overline{Y} = T\hookrightarrow_1^M
      \left(X=\overline{\kw{fun} Y : \kw{type}\Rightarrow}\kw{type}T\right):
    \left(X:\overline{(Y : \kw{type})\Rightarrow} [=\kw{type} T] \right)}\\
