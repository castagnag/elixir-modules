...
and subst_expr m = function
  ...
  | Fun (x, t, e, p) ->
     Fun (x, subst_type m t, subst_expr (Utils.SMap.remove x m) e, p)
...
let trans_module = function
  | F2.MDef (x, yt, t', e) ->
     (x, List.fold_right (fun (y, t) e -> Fun (y, t, e, I)) yt e),
     (x, List.fold_right (fun (y, t) t' -> FTy (y, t, t', I)) yt t')
  | F2.MType (x, y, t) ->
     (x, List.fold_right (fun y e -> Fun (y, Type, e, P)) y (Rei t)),
     (x, List.fold_right (fun y t -> FTy (y, Type, t, P)) y (Equ (Rei t)))
...
let trans_program = function
  | F2.B b ->
     ...
     Fun (param, sign, Rei (Sig (res @ d)) , P)
  | F2.M m ->
     ...
     let e =
       Fun (param, Sig (List.map (fun x -> x, Type) m.mparam),
